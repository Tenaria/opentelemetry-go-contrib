...
var instrumentationName = "go.opentelemetry.io/contrib/instrumentation/github.com/aws/aws-sdk-go/otelaws"

type counters struct {
	operation otelmetric.Int64Counter
}

type recorders struct {
	operationDuration otelmetric.Float64ValueRecorder
}
...
func WithTracerProvider(provider oteltrace.Tracer) Option {
	return optionFunc(func(cfg *config) {
		cfg.Tracer = provider
	})
}
...
func WithMeterProvider(provider otelmetric.Meter) Option {
	return optionFunc(func(cfg *config) {
		cfg.Meter = provider
	})
}
